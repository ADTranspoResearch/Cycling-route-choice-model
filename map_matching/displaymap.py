...
def plot_trajectory_and_path(links_gdf, row, matched_links_gdf, row_index):
    ...
    if row.geometry.geom_type == "LineString":
        ...
        gpd.GeoSeries([row.geometry]).plot(ax=ax, color='blue', linewidth=3, label='Trajectory')
        ...
        raise ValueError(f"Unsupported geometry type in trajectory: {row.geometry.geom_type}")
