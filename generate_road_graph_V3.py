...
for id_trc in road_id_list:
    ...
    first_node = int(df.loc[(df['ID_TRC'] == id_trc) & (df['vertex_pos'] == 0), 'node_id'].iloc[0])
    first_x_corr, first_y_corr = get_node_coords(id_trc, 0)

    last_node = int(df.loc[(df['ID_TRC'] == id_trc) & (df['vertex_pos'] == -1), 'node_id'].iloc[0])
    last_x_corr, last_y_corr = get_node_coords(id_trc, -1)
        

    length = float(df.loc[(df['ID_TRC'] == id_trc) & (df['vertex_pos'] == 0), 'Length'].iloc[0])
    try:
    ...
    G.add_edge(first_node, last_node, ID_RD=id_trc, length=length, bike_lane_type=0, slope=slope)
...
for id_bk in bike_id_list:
    trc = int(df.loc[(df['ID'] == id_bk) & (df['vertex_pos'] == 0), 'ID_TRC_GEO'].iloc[0])
    type1 = int(df.loc[(df['ID'] == id_bk) & (df['vertex_pos'] == 0), 'TYPE_VOIE'].iloc[0])
    type2 = int(df.loc[(df['ID'] == id_bk) & (df['vertex_pos'] == 0), 'TYPE_VOIE2'].iloc[0])
    # Applying a bike lane not on the road network
    if round(trc) not in df['ID_TRC'].values:
       

        bike_count_no_network+=1
        first_node = int(df.loc[(df['ID'] == id_bk) & (df['vertex_pos'] == 0), 'node_id'].iloc[0])
        first_x_corr, first_y_corr = get_node_coords(id_bk, 0, id_name='ID')
        last_node = int(df.loc[(df['ID'] == id_bk) & (df['vertex_pos'] == -1), 'node_id'].iloc[0])
    ...
    length = float(df.loc[(df['ID'] == id_bk) & (df['vertex_pos'] == 0), 'LONGUEUR'].iloc[0])
    try:
    ...
    if not G.has_edge(first_node, last_node):
        G.add_edge(first_node, last_node, ID_RD=id_bk, length=length, bike_lane_type=type1, slope=slope)
        G.nodes[first_node]['x'] = first_x_corr
        G.nodes[first_node]['y'] = first_y_corr
        G.nodes[last_node]['x'] = last_x_corr
        G.nodes[last_node]['y'] = last_y_corr
        ...
        if G[first_node][last_node]["bike_lane_type"]<type1:
            # TODO: check if slope is available for bike lanes if not keep road slopes for the majority
            G.add_edge(first_node, last_node, ID_RD=id_bk, length=length, bike_lane_type=type1, slope=slope)
            G.nodes[first_node]['x'] = first_x_corr
            G.nodes[first_node]['y'] = first_y_corr
            G.nodes[last_node]['x'] = last_x_corr
...
# Need to add other properties such as slope and speed adjustment
