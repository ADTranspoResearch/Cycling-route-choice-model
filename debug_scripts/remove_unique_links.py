...
def find_unique_edges(chosen_p, choice_s, graph):
    ...
    for choice in choice_s:
        ...
        for i in range(0, (len(choice) - 1)):
            ...
            rd_id_list.append(
                graph.edges[str(choice[i]), str(choice[j])]["ID_RD"]
            )
...
for index, row in cyclist_df.iterrows():
    ...
    choice_set = [
        [int(item) for item in sublist] for sublist in str_choice_set
    ]
    chosen_path = ast.literal_eval(row["trip_segment"])

    choice_set_links, unique_chosen = find_unique_edges(
        chosen_path, choice_set, G
    )
