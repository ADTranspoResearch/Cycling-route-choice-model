...
def get_path_properties(cyc_path, chosen_state=0, ini_len=0):
    ...
    lts_dict = {
        ...
        "lts_dist_w_sum": sum(lts_weight_list),
        "lts_1": lts_1,
        "lts_2": lts_2,
        "lts_3": lts_3,
        "lts_4": lts_4,
    ...
    att_dict = {
        ...
        "chosen": chosen_state,
        "not_included_links": not_included_links,
    }

    # dictionary for any attributes related to slope or level of effort
    loe_dict = {
        "avg_slope": (sum(slope_list) / max(len(slope_list), 1)),
...
trip_list = [
    f
    for f in os.listdir(choice_set_filepath)
...
for choice_set_file in trip_list:
    ...
    for choice_id, path_list in path_size_dict.items():
        ...
        for link in path_list:
            try:
                link_len = pd.Series(db_df["length"].loc[int(link)]).iloc[0]
