...
for index, row in df.iterrows():
    ...
    if cyclist_id not in unique_users:
        ...
        if len(matching_rows) != 2:
            raise ValueError(f'Expected exactly 2 rows for cyclist_id {cyclist_id}, but found {len(matching_rows)}.')
        other_row = matching_rows[matching_rows.index != row.name]
        
        if other_row.empty:
            raise ValueError(f'No other row found for cyclist_id {cyclist_id}.')
        ...
        if row['vertex_pos'] == 0 and other_row['vertex_pos'] == -1:
            ...
            raise ValueError(f'cyclist {cyclist_id} does not have origin and/or destination')
